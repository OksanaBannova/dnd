...
export default class PageController {
  constructor(pageUi, stateService) {
    this.pageUi = pageUi;
  ...
  init() {
    ...
    this.pageUi.checkBinding();
    this.pageUi.drawUi();

    this.pageUi.addCards.forEach((elem) => {
      elem.addEventListener('click', (event) => {
        event.preventDefault();

        for (const form of this.pageUi.forms) {
    ...
    Array.from(this.pageUi.cancelBtns).forEach((elem) => {
      elem.addEventListener('click', (event) => {
        event.preventDefault();
    ...
    this.pageUi.forms.forEach((item) => item.addEventListener('submit', (event) => {
      event.preventDefault();
      ...
      PageUi.createCard(column, input.value);
      item.reset();
      item.classList.remove('active');
    }));

    this.pageUi.cardsContainerEl.addEventListener('mouseover', (event) => {
      event.preventDefault();
    ...
    this.pageUi.cardsContainerEl.addEventListener('mouseout', (event) => {
      event.preventDefault();
    ...
    this.pageUi.cardsContainerEl.addEventListener('mousedown', (event) => {
  ...
  load() {
    ...
    if (data) {
      data.todo.forEach((item) => {
        PageUi.createCard(this.toDo, item);
      });
      data.inProgress.forEach((item) => {
        PageUi.createCard(this.inProgress, item);
      });
      data.done.forEach((item) => {
        PageUi.createCard(this.done, item);
      });
    }
  }
  ...
  startDrag(event) {
    ...
    this.pageUi.addCards.forEach((elem) => {
      elem.style.cursor = cursors.noDrag;
    });
    this.pageUi.cards.forEach((elem) => {
